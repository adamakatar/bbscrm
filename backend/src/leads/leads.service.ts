...
import { UploadNdaDTO } from './dto/upload-nda.dto';
@Injectable()
export class LeadsService {
  ...
  constructor(
    ...
    this.EMAIL_FROM = this.configService.get('EMAIL_FROM')
  }
  ...
  async signNda(user: IUser, signNdaDto: SignNdaDto): Promise<ILead> {
    ...
    if (!_lead.listingID.autoNdaApprove) {
      ...
      const [appConfig] = await Promise.all([
        this.appConfigService.appConfigDetails(this.EMAIL_FROM),
        ...
        .sendNdaConfirmation(
          ...
          },
          {
            ...
            webUrl: `${this.configService.get(
              'WEB_FRONT_HOSTED_URL',
            )}buy-a-business/${_lead.listingID.slug}`,
            email: appConfig?.ContactInfo?.email,
            contact: appConfig?.ContactInfo?.contact,
            address: appConfig?.ContactInfo?.address,
            url: appConfig?.ContactInfo?.url,
  ...
  async adminGetAllLeads(
    ...
    const aggregateQuery: any = [
      { $match: { ...q, status: { $in: ['under-negotiation', 'nda-signed', 'nda-approved', 'nda-submitted'] } } },
      {
        $lookup: {
          from: 'businesses',
          localField: 'listingID',
  ...
  async updateDetail(id : string, nda : UploadNdaDTO) {
    console.log(nda);
    await this.Lead.findByIdAndUpdate(id, {
      $set: { 
        nda: nda
      }
    });
