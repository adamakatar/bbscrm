...
export class DataRoomService {
  ...
  async createFolder(
    ...
    */
    ...
    // REQUIRED CODE HERE
    if(['', null, 'null', 'undefined', undefined].includes(createFolderDto?.parent) == false) {
      parentFolder = await this.Folder.findById(createFolderDto?.parent)
        .select('+isDeletable')
        .populate('business', 'broker _id')
        .lean();
    }

    if (!parentFolder && createFolderDto?.type != 'company')
    ...
    if(typeof parentFolder?.type !== 'undefined')
