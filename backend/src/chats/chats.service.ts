...
export class ChatsService {
  ...
  async sGetRooms(userId: string) {
    try {
      ...
      const ids = chat[0]?.ids || ([] as string[]);
      const rooms = await this.Room.find({ _id: { $in: ids } })
        .populate('users.userId', 'photo firstName lastName isOnline lastLogin')
        .populate('business', 'title images')
        .populate({
          path: 'lead',
  ...
  async sChatMessages(user: IUser, room: string) {
    try {
      if (!room) throw new NotFoundException('Room Id is missing');
  ...
  async sSendMessage(roomId: string, message: IReqOfSSendMessage) {
    try {
      console.log('>>>>>>>>>>> message => ', message);
      const [userFrom, _room] = await Promise.all([
        this.User.findById(message.from),
