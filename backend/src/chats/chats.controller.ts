...
export class ChatsController {
  ...
  async sChatMessages(
    ...
    try {
      const { data, totalCount } = await this.chatService.sChatMessages(user, room);
  ...
  @Put('/s-send-message/:roomId')
  // @UseGuards(AuthGuard(), RolesGuard)
  async sSendMessage(
    @Param('roomId') roomId: string,
    @Body() message: IReqOfSSendMessage
  ) {
    try {
      const resData = await this.chatService.sSendMessage(roomId, message);
  ...
  @Get('/s-get-rooms/:userId')
  @UseGuards(AuthGuard(), RolesGuard)
  async sGetRooms(
    @Param('userId') userId: string,
  ) {
    try {
      const resData = await this.chatService.sGetRooms(userId);
      return resData;
    } catch (error) {
      return ErrorHanldingFn(error);
    }
