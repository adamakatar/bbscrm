...
export class CalendarService {
   ...
   * Update an event
   */
  async updateEvent(calendarId: string, createEventDto: CreateEventDto) {
    const { date, type } = createEventDto;
    ...
    await this.Calendar.findByIdAndUpdate(calendarId, {
      ...createEventDto,
    });

    const event = await this.Calendar.findById(calendarId)
      .populate('attendees')
      .populate('customerAttendees')
      .lean();
  ...
  async deleteEvent2(eventId: string): Promise<ICalendar> {
    const event = await this.Calendar.findByIdAndDelete(eventId);
