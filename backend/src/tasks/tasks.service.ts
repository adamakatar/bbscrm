...
export class TasksService {
  constructor(
    ...
    @InjectModel('STask') private readonly STask: Model<ISTask>,
    @InjectModel('TaskTemplate')
    private readonly TaskTemplate: Model<ITaskTemplate>,
    @InjectModel('STaskStatus')
    private readonly STaskStatus: Model<ISTaskStatus>,
  ...
  async sCreateTaskStatus(name: string): Promise<ISTaskStatus | string> {
    try {
      const status = await this.STaskStatus.create({ name });
      return status;
    } catch (err) {
  ...
  async sCreateTask(reqBody: IReqBodyToCreateSTask): Promise<ISTask | string> {
    try {
      const sTask = await this.STask.create({
        ...reqBody,
        deadlineDate: new Date(reqBody.deadline),
      });
      return sTask;
    } catch (err) {
      return err.toString() || MSG_UNKNOWN_ERROR;
  ...
  async sGetAllTaskStatuses(): Promise<Array<ISTaskStatus> | string> {
    try {
      const statuses = await this.STaskStatus.find();
      return statuses;
    } catch (err) {
      return err.toString() || MSG_UNKNOWN_ERROR;
    }
