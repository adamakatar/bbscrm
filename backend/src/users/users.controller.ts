...
export class UsersController {
  ...
  @Get('/get-user-emails/:userId')
  @UseGuards(AuthGuard())
  async getUserEmails(@Param('userId') userId: string) {
    try {
      const data = await this.userService.getUserEmails(userId);
  ...
  @Get('/check-notification/:userId')
  async checkNotification(@Param('userId') userId: string) {
    const hasNotification = await this.userService.hasNotification(userId);
    return hasNotification;
  }

  @Get('/get-available-meeting-invitees/:userId')
  async getAvailableMeetingInvitees(@Param('userId') userId: string) {
    const data = await this.userService.getAvailableMeetingInvitees(userId);
    return data
  }

  @Get('/get-twillio-numbers')
  async getTwillioNumbers() {
    const data = await this.userService.getTwillioNumbers();
    return data;
  }

  @Get('/get-members')
  async getMembersOfPlatforms() {
    const data = await this.userService.getMembersOfPlatforms();
    return data;
  }
  ...
  @UseGuards(AuthGuard(), RolesGuard)
  @Roles('super-admin')
  async assignRoles(@Param('userId') userId: string, @Body() reqData: IReqOfAssignRoles) {
    const data = await this.userService.assignRoles(userId, reqData);
    return data;
  }

  @Get('/get-runtime-data/:userId')
  @UseGuards(AuthGuard(), RolesGuard)
  async getRuntimeData(
    @Param('userId') userId: string,
    @GetUser() user: IUser,
  ) {
    const data = await this.userService.getRuntimeData(userId, user);
    return data;
  }
}
