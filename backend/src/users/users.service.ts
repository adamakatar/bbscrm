...
export class UsersService {
  ...
  async updateMailFields(
    ...
    const resOfCreateSender = await this.sendgridService.createSender(
      user,
      updateImapDto.user,
      updateImapDto.nickname,
    );
    if (typeof resOfCreateSender === 'string') {
      return resOfCreateSender;
    }
    // const [_, d] = await this.mailsService.testCredentials({
  ...
  async adminCreateSpecialUser(
    ...
    let [user, appConfig] = await Promise.all([
      ...
      this.appConfigService.appConfigDetails(process.env.EMAIL_FROM),
    ]);
  ...
  async getUserEmails(userId: string) {
    try {
      const resData = await this.User.find({
        _id: { $ne: userId },
      }).select('email');
      return resData;
    } catch (error) {
  ...
  async hasNotification(userId: string) {
  ...
  async getAvailableMeetingInvitees(userId: string) {
    try {
      const users = await this.User.find({
        _id: { $ne: userId },
        role: {
  ...
  async getTwillioNumbers() {
    try {
      const accountSid = process.env.TWILIO_SID || '';
      const authToken = process.env.TWILIO_TOKEN || '';
      const twillioClient = require('twilio')(accountSid, authToken);

      const res = await twillioClient.incomingPhoneNumbers.list({ limit: 10 });
      if (res) {
        const phoneNumbers = [];

        for (let p of res) {
          const existedUser = await this.User.findOne({
            $or: [{ contact: p.phoneNumber }, { cell: p.phoneNumber }],
          });
          if (!existedUser) {
            phoneNumbers.push(p);
          }
      ...
      return error?.response?.data?.message || MSG_UNKNOWN_ERROR;
    }
  }

  async getMembersOfPlatforms() {
    try {
      const members = await this.User.find({
        $and: [{ email: /denverbbs\.com/ }, { role: { $ne: 'super-admin' } }],
      });
      return members;
    } catch (error) {
      return error?.message || MSG_UNKNOWN_ERROR;
    }
  }

  async assignRoles(userId: string, reqData: IReqOfAssignRoles) {
    try {
      await this.User.findByIdAndUpdate(userId, reqData);
      const updatedUser = await this.User.findById(userId);

      await this.communicationService.contactRegist({
        firstName: updatedUser.firstName,
        lastName: updatedUser.lastName,
        email: updatedUser.email,
        contact: updatedUser.contact,
        role: updatedUser.role.join(', '),
      });
      ...
      await this.sendgridService.sendEmail(
        ...
        'Assigned new roles',
        `<p>
          You've got new roles. Please <a href="${process.env.SITE_URL}/login">login</a> the site.
        </p>`,
      );
  ...
  async getRuntimeData(userId: string, user: IUser) {
    try {
      /** ----------- Get unread emails ------------- */
      ...
      return {
        ...
        unreadCallCount: unreadCalls.length,
        unreadSMSCount: unreadSMSes.length,
      };
    } catch (error) {
      console.log(error);
      return error?.message || MSG_UNKNOWN_ERROR;
