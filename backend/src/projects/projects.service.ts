...
import {
  IReqBodyToCreateProject,
  ISProject,
} from './interfaces/sProject.interface';
...
export class ProjectsService {
  constructor(
    ...
    @InjectModel('SProject') private readonly SProject: Model<ISProject>,
  ...
  async getSingleProject(slug: string): Promise<IProject | string> {
    try {
  ...
  async addProject(
    ...
    const project = await this.Project.create({ ...createProject, slug, isNew: true });
    const populatedProject = this.Project.findById(project?._id)
                        .populate([{ path: 'business' }, { path: 'assignTo' }]);
  ...
  async switchTask(user: IUser, payload: SwitchTaskDto): Promise<IProject> {
    ...
    const _project = await this.Project.findOne({ _id: project._id })
        .populate([
          {
            path: 'stages.tasks',
            populate: { path: 'assignedTo' },
  ...
  async sCreateProject(
    reqBody: IReqBodyToCreateProject,
  ): Promise<ISProject | string> {
    try {
      const sProject = await this.SProject.create({
        ...reqBody,
        isActive: true,
      });
      return sProject;
    } catch (err) {
      return err.toString() || MSG_UNKNOWN_ERROR;
    }
  }

  async sGetAllProjects(): Promise<Array<ISProject> | string> {
    try {
      const sProjects = await this.SProject.find();
      return sProjects;
    } catch (err) {
      return err.toString() || MSG_UNKNOWN_ERROR;
    }
