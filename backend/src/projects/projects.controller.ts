...
export class ProjectsController {
  ...
  @Post('/s-create-project')
  async sCreateProject(@Body() reqBody: IReqBodyToCreateProject) {
    try {
      const data = await this.Projectservice.sCreateProject(reqBody);
      return data;
    } catch (err) {
  ...
  @Get('/s-get-all-projects')
  async sGetAllProjects() {
    try {
      const data = await this.Projectservice.sGetAllProjects();
      return data;
    } catch (err) {
      throw ErrorHanldingFn(err);
    }
