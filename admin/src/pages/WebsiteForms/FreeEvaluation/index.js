...
import classes from "./FreeEvaluation.module.css";
import {
  BaseURL,
  recordsLimit,
...
const FreeEvaluation = () => {
  ...
  const [freeEvaluation, setFreeEvaluation] = useState([]);
  ...
  const getAllFreeEvaluation = async (pg = page) => {
    const url = BaseURL(
      `newsletters/admin/all?type=free-business-evaluation&page=${pg}&limit=${recordsLimit}&search=${search}`
    );
    setLoading(true);
    const response = await Get(url, accessToken);
    if (response !== undefined) {
      setFreeEvaluation(response?.data?.data);
  ...
  useEffect(() => {
    getAllFreeEvaluation();
  }, [page]);

  useEffect(() => {
    setPage(1);
    getAllFreeEvaluation(1);
  }, [debouncedSearchTerm]);
          ...
          <div class="table100 ver1 m-b-110">
            ...
            {loading ? (
              ...
              <div class="table100-body js-pscroll ps ps--active-y">
                <table>
                  <tbody>
                    {freeEvaluation?.length > 0 ? (
                      freeEvaluation?.map((item, index) => (
                      ...
                      <NoData text={"No Valuations Found"} />
                    )}
                  </tbody>
                </table>
              </div>
