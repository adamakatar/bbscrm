...
import classes from "./LiquorAPA.module.css";
...
const CustomInput = (props) => {
  ...
  return (
    <input
      ...
      type={type}
      className={`${[classes.inputBox, className && className].join(" ")}`}
      style={{
        borderBottomWidth: value?.length == 0 ? 0 : 1,
      }}
    />
  );
};
  ...
  const sendPdf = async () => {
    ...
    if (response !== undefined) {
      downloadFileFromUrl(
        ...
        `${data?.buyer_name}-${data?.business_title}-Liquor-APA.docx`
      );
    ...
    <>
      ...
      <div className={classes.mainContainer} ref={reportTemplateRef}>
        ...
        <div>
          ...
          <p>
            Buyer shall purchase all unexpired and marketable inventory of the
            Business at Seller's cost in cash at the time of closing. Seller’s
            cost may be verified by invoices at Buyer's option. The inventory is
            to be taken prior to closing by an independent third-party that will
            be paid as follows: ½ by Buyer and ½ by Seller. Seller’s cost shall
            be determined as follows: retail less{" "}
            {allValues?.lastNumberOfPara1_manual} %, except for beer kegs, which
            shall be at wholesale.
          </p>
          <p>
            <strong>2. </strong>
          ...
          <p>
            ...
            <strong>
              Buyer and Seller understand that this transaction is based on a
              temporary liquor permit as opposed to a permanent license and
              accept the risk of same.
            </strong>
          </p>
          <p>
          ...
          <p>
            ...
            indirectly engage in the business of owning or operating a retail
            liquor store within a {allValues?.mileRadius} mile radius of the
            Premises for a period of {allValues?.mileRadiusYears} years. Current
            businesses owned and operated by the Seller and its Principals shall
            be exempt from this radius.
          </p>
          <p>
            <strong>16. </strong>
